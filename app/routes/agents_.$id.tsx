import { Form, Link, useActionData, useLoaderData, useNavigate, useParams, useSubmit } from "@remix-run/react";
...
import { updateAgent } from "~/resolvers/agents";
...
export async function loader({ request }: LoaderArgs) {
    try {
        ...
        const agentData = {
            ...ddata.payload,
            published: z.coerce.boolean().parse(ddata.payload.published.toLowerCase() === 'true'),
          };
...
export async function action({ request }: ActionArgs) {
    ...
    try {
        console.log(ctx)
        // const payload = await updateAgent({ ctx })
...
export default function Agents() {
    ...
    const [checked, setChecked] = useState(agentsList.published)
    ...
    function handleSubmit({ formValues }: { formValues: AgentType }) {
        ...
        submit(formData, { method: 'post', action: `/agents/${id}` });
    }
            ...
            <div className="max-w-2xl w-full">
                ...
                <Form onSubmit={handleFormSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    <div className="flex items-center justify-between mb-4">
                        <h1 className="text-2xl font-bold capitalize">Edit {agentsList?.name}</h1>
                        <Link to={'/agents'}>
