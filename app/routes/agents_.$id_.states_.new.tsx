...
import { createState } from "~/resolvers/states";
...
export async function action({ request, params }: ActionArgs) {
    ...
    try {
        console.log(ctx)
        const payload = await createState({ ctx })
...
export default function Agents() {
    ...
    useEffect(() => {
        const newState = localStorageService.getItem('newState')
        setFormValues({...formValues, name: newState})
    ...
    function handleSubmit({ formValues }: { formValues: StateType }) {
        ...
        submit(formData, { method: 'post', action: `/agents/${id}/states/new` });
    }
            ...
            <div className="max-w-2xl w-full">
                ...
                <Form onSubmit={handleFormSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    <div className="flex items-center justify-between mb-4">
                        <h1 className="text-2xl font-bold capitalize">New {agentName} State</h1>
                    ...
                    <div className="mb-4 flex items-center">
                        <Label htmlFor="type_" className="w-32">Type</Label>
                        <Input
                            name="type_"
                            type="text"
